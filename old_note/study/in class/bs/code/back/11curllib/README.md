...
void Game_Paint(HWND hwnd){
  ...
  std::list<BitmapInfo> bitmap_info_list;
	...
	if (YGlobalVars::flag_compiled_ == 1 && YGlobalVars::flag_compile_success_ == 0){
		...
		HFONT hFont = CreateFont(30, 0, 0, 0, 0, 0, 0, 0, 0/*GB2312_CHARSET*/, 0, 0, 0, 0, L"微软雅黑");  //创建一种字体
		...
		if (YGlobalVars::compile_info_ == "28"){//the timeout time was reached.
			text1 = L"请求超时，请检查网络连接";
		}
		else{
			text1 = L"abc";
		}
		//SetTextColor(YGlobalVars::hMdc, RGB(50, 255, 50));
		//TextOut(YGlobalVars::hMdc, 30, 150, text1, wcslen(text1));
		TextOut(YGlobalVars::hMdc, YGlobalVars::view_result_rect_.left,YGlobalVars::view_result_rect_.top, text1.c_str(), wcslen(text1.c_str()));
