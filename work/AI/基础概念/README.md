...
3）R1-Distill-Qwen-32B：经过蒸馏（Distill（训练数据来源））训练的，可以理解成就是先让牛的模型（R1，有685B）做1万道数学题，并写出详细的思考过程，把这些"大师解题思路"整理成教案，让学徒（最初是Qwen-32B这个小模型）对着这些教案反复学习微调（Fine-tuning）（这个模型是用的SFT全量监督微调（训练方式，还有LoRA部分微调）），最终学徒不仅学会了答案，还学会了大师的思考方式，最终成为了只有32B的参数，但是能力却能接近685B参数；
