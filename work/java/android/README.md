安装android studio（自带java环境（安装目录jbr\bin下，如果不想配置环境变量， 就需要在这个目录执行命令）、android sdk（AppData\Local\Android\Sdk下））

- APK 
Android Package Kit（安卓安装包），本质是一个 ZIP 格式的归档文件

- apktool
（要自己下载）
APK虽然能直接被unzip解压，但是apktool更强大，能反编译、解压、重打包；apktool d 解包后的目录结构是精心设计的
java.exe -jar apktool_2.12.0.jar d game.apk -o decode/
当你修改完代码和资源后（META-INF文件的不需要修改，apktool会自动帮改），可以使用 apktool b 命令重新打包成一个新的 APK 文件；
java.exe -jar apktool_2.12.0.jar b decode/ -o game2.apk
然后还必须签名（自己的私钥，在已经安装官方版本APK的情况下原地安装系统会因为签名不一致而拒绝安装，所以需要先卸载原APK）
（在sdk中，build-tool里）
zipalign.exe -p -f 4 game2.apk game3.apk
apksigner.bat sign --ks my-release-key.jks --ks-key-alias my-key-alias --ks-pass pass:123456 --key-pass pass:123456 --out game4.apk game3.apk

- jks生成方法：
（在studio里）keytool.exe -genkey -v -keystore my-release-key.jks -keyalg RSA -keysize 2048 -validity 10000 -alias my-key-alias

- classes.dex
编译后的 Android 字节码文件。你的 Java/Kotlin 源代码最终都被编译成了这个格式
其他工具（如 dex2jar + jd-gui） JADX 可以直接将 classes.dex 反编译成可读的 Java 代码（尽管可能不是原始的完美代码）

- adb
（在sdk中，platform-tools；有些其他工具也自带adb，例如scrcpy）

- aapt 
（在sdk中，build-tools）
dump badging <your.apk>  # 定位并读取某安装包里 AndroidManifest.xml的属性，比apktool轻量级；例如package: name后面是包名

- jadx-gui
（要github下载）
可以反汇编classes.dex文件变成java（仅用于分析代码，但会丢失信息）

- 想调试apk的话，
先apktool 解包，然后修改AndroidManifest.xml，在<application 这里> 添加android:debuggable="true"

- 查看文件是否被篡改&使用的证书指纹：
apksigner.bat verify --verbose --print-certs <your.apk>
（保护1）显示出来的Signer #1 certificate SHA-256就是实时计算出来的证书指纹（开发者一般也会在官网公布自己的证书指纹，所以用户下载到APK之后可以比较这个SHA256确认这个证书是对的/官方的）
（保护2）如果已安装了某app，再安装（例如升级或者更新）同包名的app时，如果发现证书指纹不一致，系统会拒绝安装；
（保护3）显示的Verify=true就表示文件没坏。系统安装时也会校验。原理：
v2/v3都是把证书+数字签名等信息写入APK文件尾的一个区块（v1情况可能不同（有META_INFO文件）暂且不提）。其中数字签名是对APK文件除签名块以外的部分使用固定的规则算哈希H1，然后用私钥签名得到。（相当于除了证书里面本身有一部分是证书的数字签名，证书外面还有一个APK的数字签名）
系统安装时从APK提取出证书里的公钥可以解签名得到H1，然后再对APK文件除签名块以外的部分使用固定的规则算哈希H2，对比H1是否等于H2即可。

总结上述所有保护，只要用户第一次安装前比较了证书指纹是官方的，并且官方私钥没有泄漏，那么就可以放心使用这个APP。

- 想root安卓模拟器的话
android studio 启动一个google api的就行，不要启动google play的

- ida
逆向代码
下载：https://www.52pojie.cn/forum.php?mod=viewthread&tid=1581672&extra=page%3D1&page=1

- 想ida pro调试安卓模拟器的话（没跑通）
需要root安卓模拟器，并且把android_server64用adb传过去，然后adb shell 过去./启动
然后用adb启动（等待debug模式的）app
adb shell am start -D -ncom.tianxaing.release/com.fish.main.MainGameActivity





## 一些参考
- https://blog.csdn.net/qq_42177292/article/details/121261627?spm=1001.2101.3001.6650.10&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ERate-10-121261627-blog-147095615.235%5Ev43%5Epc_blog_bottom_relevance_base4&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ERate-10-121261627-blog-147095615.235%5Ev43%5Epc_blog_bottom_relevance_base4&utm_relevant_index=19

- 修改smail
https://blog.csdn.net/weixin_37618354/article/details/149749516

- https://www.freebuf.com/author/zzwlpx
