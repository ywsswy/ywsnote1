...
```
有几个描述符（软链接）
012开始时都是指向屏幕的，即软链接/proc/self/fd/[0/1/2]指向相同，即标准输出、标准错误(键盘敲的实时回显）都能在屏幕看得到；
...
因为前者是ln -sf /proc/self/fd/1 <file>，然后ln -sf /proc/self/fd/2 /proc/self/fd/1，就相当于两个都指向一个文件了，
